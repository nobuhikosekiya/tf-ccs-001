...
curl -s -X GET "$LOCAL_ENDPOINT/_data_view/remote_all_indices?pretty" \
  -u "$CCS_USER:$CCS_PASSWORD"

# 7. Check Kibana access (should return 200 OK if permissions are correct)
echo -e "\n--- Testing Kibana access ---"
curl -s -I -X GET "$LOCAL_ENDPOINT/api/status" \
  -u "$CCS_USER:$CCS_PASSWORD" \
  -H "kbn-xsrf: true"
...
DATA_VIEW_CHECK=$(curl -s -o /dev/null -w "%{http_code}" "$LOCAL_ENDPOINT/_data_view/remote_all_indices" \
  -u "$CCS_USER:$CCS_PASSWORD")
...
# Check Kibana access
KIBANA_CHECK=$(curl -s -o /dev/null -w "%{http_code}" "$LOCAL_ENDPOINT/api/status" \
  -u "$CCS_USER:$CCS_PASSWORD" \
  -H "kbn-xsrf: true")
